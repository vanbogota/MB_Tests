...
namespace AreaCalculatorsTests
{
    public class TriangleFigureUnitTests
    {
        ...
        [Fact]
        public void If_Triangle_IsNot_Triangle_Throws_Exception()
        {
            Exception ex = Assert.Throws<Exception>(() => new Triangle(1, 1, 125));
            Assert.Equal("It is not a triangle", ex.Message);
        }
